...
- (void)setContact:(Contact *)aContact
{
    ...
    else if (contact.fullAddress != nil && ![contact.fullAddress isEqualToString:@""])
    {
    ...
    else if (contact.phone != nil && ![contact.phone isEqualToString:@""])
    {
    ...
    else if (contact.email != nil && ![contact.email isEqualToString:@""])
    {
        [self addEntry:contact.email withLabel:@"Email"];
        [self addSectionBreak];
    }
    
    if (contact.company != nil && ![contact.company isEqualToString:@""])
    {
...
- (NSImageView *) addPortrait:(NSImage *)image
{
    NSSize size = self.view.frame.size;
    
    NSImageView *imageField = [[NSImageView alloc] initWithFrame:
                               NSMakeRect(labelIndent, size.height - 72, 63, 63)];
